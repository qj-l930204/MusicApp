<template>
  <transition name="list-fade">
    <div class="playlist" v-show="showFlag" @click="hide">
      <!-- @click.stop阻止lpaylist点击事件的冒泡 -->
      <div class="list-wrapper" @click.stop>
        <div class="list-header">
          <h1 class="title">
            <i class="icon" :class="iconMode" @click="changePlayMode"></i>
            <span class="text">{{playText}}</span>
            <span class="clear" @click="showConfirm">
              <i class="icon-clear"></i>
            </span>
          </h1>
        </div>
        <Scroll :data="sequenceList" class="list-content" :refreshDelay="refreshDelay" ref="listContent">
          <transition-group name="list" tag="ul">
            <li ref="listItem" class="item" v-for="(item, index) in sequenceList" @click="selectPlaySong(item, index)" :key="index">
              <i class="current" :class="getCurrentIcon(item)"></i>
              <span class="text" :class="getCurrentColor(item)">{{item.name}}</span>
...
//        const index = this.playList.findIndex((currentsong) => {
